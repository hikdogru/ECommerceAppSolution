...
{
    public class BrandService : CRUDService<IRepository<Brand, ObjectId>, Brand, ObjectId>, IBrandService
    {
        public BrandService(IRepository<Brand, ObjectId> repository) : base(repository)
        {
        }

        public async Task<PaginatedList<BrandDTO>> Filter(int page, int pageSize, GridFilters? filter = null)
        {
            ...
            var pagedData = await allData.ProjectTo<BrandDTO>(MapperConfig.GetMapper().ConfigurationProvider)
